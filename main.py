...
def load_data():
    df = pd.read_excel("Songs_with_MFCC_Features_2_segments_Final_Result_Reco_engine.xls")
    track_df = df.iloc[:,42:]
    return track_df

track_df = load_data()
#songs_path = "D:\\m.tech_downloads\\Capstone_Project\\Data\\Veera_Rasa\\wav_cleaned_VeeraRasa\\Meghamalhaar raga\\"
...
with st.container():
    ...
    with col2:
        file_ = open("MusicData/listening-to-music-spongebob.gif", "rb")
        contents = file_.read()
...
with st.container():
    ...
    with col1:
        ...
        rasa = st.radio(
            "",
            rasa_names, index=rasa_names.index("peace/tranquility"))
